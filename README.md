...
- Bouton-favori : `npm run build -w apps/web` construit d'abord `public/capture.js` (règles incluses, adresse de production ou de l'aperçu Cloudflare Pages, ou `LOUPE_BASE_URL`) ; la page `/extension` le propose à glisser dans la barre de favoris.
...
- Tests : Vitest + Testing Library (jsdom), couverture 100 % sur `stockage/`, `formatage/`, `textes/`, `annonces/`, `hypotheses/`, `analyses/`, `bookmarklet/`.
