...
1. **Base D1** : fait le 14/09/2026. La base `deklic-comptes` est créée dans la juridiction UE (`npx wrangler d1 create deklic-comptes --jurisdiction eu`), son identifiant est dans `apps/comptes/wrangler.toml` et la migration 0001 est appliquée. Nouvelle migration : `npx wrangler d1 migrations apply deklic-comptes --remote` depuis `apps/comptes`. Migrations `0002_gestion.sql` (gestion locative) et `0004_projets.sql` (projets synchronisés) appliquées le 14/09/2026. **À faire, avant de fusionner la PR `feat/quittances-fiches`** : `0003_gestion_documents.sql` (paiements en partie, quittances et reçus, colocation), par cette même commande ; sans elle, une fois ce code déployé, Gérer répond « pas disponible ». Pour mémoire, sans les migrations, Gérer indique « pas disponible » aux comptes connectés et Mes projets « sauvegarde du compte indisponible » (les projets restent sur l'appareil).
