...
- Coque d'application (feature `coque-fixe`, [architecture](.product/architecture/coque-fixe.md)) : la fenêtre ne défile jamais, seul le contenu défile. Barre latérale de 224 px en trois zones (logo et « Accueil » en haut, le logo mène aussi à l'accueil ; sections « Analyser » — une ligne « Mes projets · N » et son « + » pour créer un projet (Comparer est dans la liste), puis trois projets récents — et « Gérer » — « Mes biens · N » et son « + » pour ajouter un bien —, seule zone qui défile ; simulateur de prêt, méthode, extension, « Installer l'application » et profil en bas, toujours visibles) ; sous 1 024 px, elle devient un tiroir ouvert depuis une barre d'app. Dans un projet, l'en-tête compact (nom · prix · mode, statut choisi dans une liste aux couleurs de Deklic, feuille du bas sur téléphone, PDF, Partager, onglets Rapport, Estimation, Financement, Hypothèses, Fiscalité, Revente, Visite) reste collé en haut du contenu : sur téléphone, seule la bande des onglets reste ; la synthèse d'Hypothèses se colle dessous ; changer de page remet le contenu en haut.
- **Accueil** (`/`, feature `accueil-menu`, [architecture](.product/architecture/accueil-menu.md)) : un bloc par section du menu (côte à côte à partir de 1 024 px si Analyser et Gérer sont affichées). Analyser : sans projet à soi (ou avec le seul exemple jamais modifié), « Analyser une annonce » et « Voir l'exemple » ; sinon le nombre de projets à l'étude, les étapes (en analyse, visite prévue, offre faite, acheté), le meilleur cash-flow et une prochaine étape écrite par règles (acheté → le gérer, offre → vérifier le financement, visite prévue → préparer la visite, en analyse → voir le rapport). Gérer : sans compte, « Se connecter » ; sans bien, « Ajouter mon premier bien » ; avec des biens, les loyers reçus du mois, la barre des montants et les retards. Mon compte → « Mon menu » : Analyser et Gérer, Analyser seulement ou Gérer seulement.
- Écrans livrés : **Nouveau projet** (lien d'annonce reconnu sur LeBonCoin, SeLoger, Bien'ici, PAP, Logic-Immo ; page lue par l'extension, le bouton-favori ou, sans extension, par Deklic lui-même ; texte de l'annonce collé et lu par règles ; ou saisie manuelle ; formulaire Vérifier avec provenance de chaque valeur, où seuls prix, surface, code postal et ville sont exigés : apport, durée et tranche sont pré-remplis « estimé », le loyer vide vient des loyers de marché de la commune ; sans loyer, le rapport dit « Il manque le loyer visé pour cette analyse » et offre le champ sur place), **Mes projets** (liste, filtres, statut, suppression), **Rapport** (verdict, cinq feux, autofinancement en carte principale : cascade loyer → après le crédit → après les charges → après l'impôt avec ses trois repères, prix vs ventes réelles, rendements brut · net · net-net, leviers, fiscalité, revente avec multiple sur apport ; une icône ⓘ par titre et par repère ouvre une bulle chiffrée, un lien « Voir … → » mène à l'onglet qui détaille), calculés par le moteur, et **Extension navigateur** (bouton-favori à glisser, guide de l'extension).
...
- **Lecture sans extension** (feature `lecture-serveur`, [ADR-008](.product/adr/008-lecture-serveur.md), [architecture](.product/architecture/lecture-serveur.md)) : sur téléphone ou sans extension, coller le lien suffit. Le Worker rapporte la page de l'annonce (`POST /lecture`), le navigateur y applique les mêmes règles que l'extension (`src/annonces/lecture-serveur.ts`, document inerte, HTML gardé en mémoire seulement). Pendant l'attente (5 à 75 s), un écran montre l'étape, une progression estimée d'après la durée habituelle du portail, le temps écoulé et une astuce, avec « Annuler » ; il s'affiche dès que le lien est reconnu. Si l'extension n'arrive pas à ouvrir l'annonce, Deklic prend le relais ; sinon « Lire sans l'extension ». Lecture impossible (Worker indisponible, annonce retirée, annulée) : le message le dit, avec « Réessayer la lecture » et « Saisir à la main » (le lien reste la source du projet). Il n'y a plus de zone pour coller le texte de l'annonce ; un texte partagé sans lien depuis le téléphone est lu dès l'ouverture.
- **Le bien** : une annonce lue garde ses photos (adresses sur le portail, jamais copiées, affichées sans référent et seulement depuis les cinq portails) et sa fiche (chauffage, état, étages, extérieurs, cave, parking, équipements, budget énergie, honoraires, vendeur, quartier, date de publication) avec le projet, dans le lien de partage aussi ; carte « Le bien » en tête du Rapport. L'état, l'extérieur et les honoraires à la charge de l'acquéreur remplissent le formulaire Vérifier.
...
- Compte optionnel : page **Connexion** (`/connexion`, hors de la coque : Google, Apple, code par e-mail), page **Mon compte** (`/compte` : nom, projets, menu, déconnexion, suppression), profil dans la barre latérale ; client `src/compte/` (voir « Les comptes »).
...
- Règles de lecture par portail dans `apps/extension/regles/<portail>.json` (versionnées `<portail>-AAAA-MM-JJ`) : pour chaque champ, JSON-LD schema.org, puis état applicatif de la page, puis balises `og:`, puis sélecteurs CSS. Un portail qui change de maquette se corrige en éditant un JSON et sa fixture. Règles `-2026-09-14` relevées sur une vraie annonce par portail (lue par Bright Data) : photos, chauffage (collectif ou individuel, énergie), état, étages de l'immeuble, extérieurs, cave, parking, équipements, salles d'eau, honoraires et à qui ils incombent, budget et consommation d'énergie, date du DPE, quartier, vendeur pro ou particulier, date de publication ; fixtures réduites et anonymisées `tests/fixtures/*-complet.*`.
- `@loupe/capture` : `CaptureSchema` (version 1, champs enrichis optionnels, mode `serveur`), `FicheAnnonceSchema`, `encoderCapture` / `decoderCapture` (base64url, jamais d'exception), `resoudreAnnonce` (aussi exporté seul : `@loupe/capture/portails`, sans types DOM, pour le Worker), moteur de règles (`capturer`, `creerRegistre` ; chemins `[*]`, types `urls` et `date`). Tests : 108 pour le contrat et le moteur de règles, 67 pour l'extension (règles sur pages enregistrées, popup avec un faux `chrome`), couverture 100 %.
...
- Hono sur Cloudflare Workers. Routes : `GET /health` ; `GET /proxy/geocodage?q=…&limit=…&codePostal=…` (Géoplateforme IGN), réponse au contrat Loupe : libellé, score, latitude/longitude, précision (adresse, rue, lieu-dit, commune), clé BAN, code INSEE ; `POST /extract { texte }` : un modèle de langage lit le texte d'une annonce et rend 23 champs (prix, surface, étage, ascenseur, DPE, charges, taxe foncière, année, lots, loyer actuel, état du bien, balcon ou terrasse, type de location — colocation, courte durée, moyenne durée, meublée ou nue, seulement si l'annonce le dit…), chacun validé ou `null`, jamais inventé (prompt v3) ; `GET /marche?codeInsee=…&codePostal=…&type=…&pieces=…` : médiane et quartiles des ventes DVF, loyer d'annonce ANIL et zone ABC de la commune (ou de l'arrondissement désigné par le code postal), lus dans le bucket R2 `deklic-data` et gardés en cache 24 h, avec la source de chaque valeur, la date de la vente médiane et l'ancienneté des ventes en mois (contrat v2) ; `GET /marche/adresse` (contrat v4) rend en plus, pour le repère et chaque groupe, la période et la date médiane des ventes comparables, et l'ancienneté du repère à la date de la réponse. Le contrat v6 ajoute `ventesCarte` : les ventes comparables géolocalisées à 300 m au plus, pour la carte. `POST /lecture { url }` : la page d'une annonce des cinq portails (voir ci-dessous). `GET /proxy/communes?codePostal=…` ou `?nom=…` : communes de l'API Géo pour le champ « Commune » du formulaire (nom, code INSEE, codes postaux ; gardées 30 jours par code postal, recherche par nom jamais gardée). Version du Worker : 0.11.0.
- Lecture d'une annonce sans extension (ADR-008) : `POST /lecture` n'accepte que les pages de Deklic (origine vérifiée), un lien d'annonce reconnu en https et un corps de 2 Ko au plus ; Bien'ici est lu directement (`/realEstateAd.json`), les quatre autres portails par Bright Data Web Unlocker (secret `BRIGHTDATA_API_KEY`, zone `BRIGHTDATA_ZONE` = `deklic_unlocker`), deux tentatives au plus (page vide ou sans données), 404 du portail → `ANNONCE_INTROUVABLE`. Rien n'est mis en cache ni journalisé du contenu ; 5 lectures par minute et par IP (`LIMITEUR_LECTURE`). Sans clé : `503 LECTURE_INDISPONIBLE` (sauf Bien'ici). Compte Bright Data sans fonds déposés : plafond de coût = 5 000 lectures gratuites par mois.
...
- Erreurs en codes (`SERVICE_INCONNU`, `PARAMETRES_INVALIDES`, `TROP_DE_REQUETES`, `AMONT_INDISPONIBLE`, `AMONT_SATURE`, `AMONT_INVALIDE`, `AMONT_VIDE`, `ANNONCE_INTROUVABLE`, `LECTURE_INDISPONIBLE`, `ORIGINE_REFUSEE`) : l'interface les traduit. Journal structuré (une ligne JSON par événement), sans adresse IP ni adresse saisie.
...
3. Lecture sans extension (une fois) : `npx wrangler secret put BRIGHTDATA_API_KEY` (clé de la zone « Web Unlocker » `deklic_unlocker` du compte Bright Data ; ne jamais la coller ailleurs). Vérifier : `GET /health` répond `"lecture": "brightdata"`.
...
- **Quittances et fiches** (feature `quittances-fiches`, [architecture](.product/architecture/quittances-fiches.md)) : paiement en partie (la somme du mois ne dépasse jamais le dû, garanti par une insertion conditionnelle ; date future refusée), `PUT /bailleur` (nom et adresse, demandés une seule fois), `POST /documents` (quittance d'un mois entièrement reçu, reçu d'un paiement partiel ; contenu figé, même document si on le redemande), `GET /documents/:id`, `POST /locations/:id/fin` (sortie du locataire), `POST /biens/:id/locations` (louer un bien vacant ou une autre chambre, colocataires compris, 50 locations par bien), `GET /export` (toutes les données de gestion en JSON). Un paiement attesté par un document ne s'annule plus. Migration `0003_gestion_documents.sql`. Web : page Loyers `/gerer/loyers`, document imprimable `/gerer/documents/:id`, fiche d'un bien `/gerer/biens/:id`, carte « Mes données de gestion » dans Mon compte.
- **Mes biens et vie de la location** (feature `gerer-biens`, [architecture](.product/architecture/gerer-biens.md)) : `PATCH /locations/:id` (nouveaux montants à partir d'un mois, refusés par une insertion conditionnelle si ce mois ou un mois suivant a déjà reçu un paiement : 409 `PERIODE_PAYEE` ; jour du loyer, dépôt, chambre), `DELETE /biens/:id` (le bien, ses locations, changements, colocataires, paiements et documents, et les locataires sans autre location, en un lot), `PATCH /locataires/:id` (nom et e-mail ; les documents émis gardent l'ancien nom), APL versée au bailleur à la création, à la location et à la modification (au plus le loyer charges comprises, comprise dans le total, détaillée sur la quittance). Migration `0005_gestion_changements.sql`. Web : pages Mes biens `/gerer/biens` et Mes locataires `/gerer/locataires`, « Modifier » et « Supprimer ce bien » sur la fiche d'un bien.
...
- **Liens de partage courts** (feature `liens-courts-domaine`, [ADR-009](.product/adr/009-liens-de-partage-courts.md), [architecture](.product/architecture/liens-courts-domaine.md)) : `POST /api/partage` (sans compte, `Origin` connu exigé, corps ≤ 64 Ko, 10 créations par heure par IP) enregistre une copie du projet **sans la visite** et rend `{ id, jeton, expireLe }` ; `GET /api/partage/:id` la rend et repousse l'expiration à 90 jours ; `DELETE /api/partage/:id` avec le jeton l'éteint. Table `partage` de la migration `0006_partage.sql` (jeton et IP en empreintes SHA-256, IP effacée après une heure, liens expirés purgés à chaque création) ; base non migrée : 503 `PARTAGE_INDISPONIBLE` et le bouton Partager donne le lien long compressé `/partage#z=…`. Web : route `/p/:id`, `src/stockage/{partage,partage-client,liens-partage,compression,transfert}.ts`, page `/transfert`.

### Passer sur app.deklic.pro (Pierre, dans l'ordre)

Le code est prêt ; tant que ces étapes ne sont pas faites, rien ne change (adresse `loupeprojet.pages.dev`, aucune redirection).

1. **Migration des liens courts** (avant de fusionner la PR de la feature, indépendante du domaine) : depuis `apps/comptes`, `npx wrangler d1 migrations apply deklic-comptes --remote` (applique `0006_partage.sql`, et `0005_gestion_changements.sql` si elle manque encore).
2. **Domaine** : acheter `deklic.pro`, zone DNS sur Cloudflare ; projet Pages `deklic`, Custom domains, ajouter `app.deklic.pro` (certificat automatique). Vérifier que `https://app.deklic.pro` affiche Deklic.
3. **Google** : sur le client OAuth, ajouter l'URI de redirection `https://app.deklic.pro/api/auth/callback/google` (garder l'ancienne).
4. **Apple** (si configuré) : ajouter le domaine `app.deklic.pro` et le retour `https://app.deklic.pro/api/auth/callback/apple` au Services ID (garder les anciens).
5. **Resend** : vérifier le domaine `deklic.pro` (SPF, DKIM), puis `COURRIEL_EXPEDITEUR` = `Deklic <connexion@deklic.pro>` dans le projet Pages.
6. **Worker** : `npm run deploy -w apps/worker` (une seule fois, avec les autres changements du Worker de la nuit) pour que le CORS accepte `app.deklic.pro`. Les comptes l'acceptent déjà.
7. **Tester la connexion** sur `https://app.deklic.pro` (Google, code e-mail).
8. **Variables de build** du projet Pages (Production) : `DEKLIC_ORIGINE` = `https://app.deklic.pro` (bouton-favori, image de partage), puis relancer un déploiement.
9. **Extension** : `DEKLIC_ORIGINE=https://app.deklic.pro npm run build -w apps/extension`, recharger ou republier la version 0.4.0 (identifiant Firefox inchangé).
10. **Bascule** : quand tout ce qui précède fonctionne, `DEKLIC_TRANSFERT` = `1` dans les variables de build, relancer un déploiement. L'ancienne adresse envoie alors une fois les projets de chaque appareil vers `https://app.deklic.pro/transfert`, puis redirige toutes les visites. Les aperçus `*.loupeprojet.pages.dev` ne sont pas redirigés. Garder l'ancienne adresse au moins 12 mois.
11. Retour arrière : retirer `DEKLIC_TRANSFERT` et relancer un déploiement (rien n'a été supprimé sur l'ancienne adresse).
...
1. **Base D1** : fait le 14/09/2026. La base `deklic-comptes` est créée dans la juridiction UE (`npx wrangler d1 create deklic-comptes --jurisdiction eu`), son identifiant est dans `apps/comptes/wrangler.toml` et la migration 0001 est appliquée. Nouvelle migration : `npx wrangler d1 migrations apply deklic-comptes --remote` depuis `apps/comptes`. Migrations `0002_gestion.sql` (gestion locative), `0003_gestion_documents.sql` (paiements en partie, quittances et reçus, colocation) et `0004_projets.sql` (projets synchronisés) appliquées le 14/09/2026 (sous Windows, `npx.cmd` si PowerShell refuse les scripts). **À faire, avant de fusionner la PR `feat/gerer-biens`** : `0005_gestion_changements.sql` (loyers modifiés à partir d'un mois, APL versée au bailleur), par cette même commande ; sans elle, une fois ce code déployé, Gérer ne répond plus. Appliquée avant la fusion, elle ne gêne pas le code en production (une colonne ignorée, une table pas encore lue). Pour mémoire, sans les migrations, Gérer indique « pas disponible » aux comptes connectés et Mes projets « sauvegarde du compte indisponible » (les projets restent sur l'appareil).
