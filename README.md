...
- Onglet **Adresse** : une fois l'adresse exacte connue (agence, diagnostics), le Worker (`GET /marche/adresse`) classe les ventes réelles de la commune en même immeuble (parcelle cadastrale), parcelles voisines (API Carto de l'IGN), même côté de la rue et en face (code de voie et parité des numéros), cercles de 100, 200 et 300 m ; comparables de même type et de surface proche ; repère de prix sur le groupe le plus précis qui compte au moins 5 ventes, avec l'écart du bien, et bouton « Utiliser ce repère pour le verdict ». Aucun modèle de langage. L'adresse est gardée avec le projet ; l'analyse est refaite à l'ouverture.
