...
- **Prix** : médiane €/m² DVF 500 m / 24 mois, ±10 % selon surface ; fourchette Q1–Q3. Depuis `estimation-confiance` : une **note de confiance sur 100** accompagne l'estimation (localisation du repère 35, dispersion des prix 30, ventes comparables 20, ancienneté des ventes 15 ; cinq niveaux, marge de la fourchette de ±5 % à ±15 % selon le niveau), affichée en tête de l'onglet avec ses raisons ; sans adresse, le repère de commune ou d'arrondissement est montré tel quel (« moins précis »). Depuis `adresse-suggestions` : l'adresse se choisit dans des suggestions pendant la frappe (département du projet d'abord ; choisir lance l'analyse ; une rue demande le numéro, avec « Je ne connais pas le numéro ») et les **adresses du cadastre** (numéros fictifs 9xxx de la DGFiP, résidences et cités absentes de la BAN) sont proposées à partir des ventes DVF de la commune, puis analysées au numéro fiscal et au code de voie (groupe « Même immeuble »). Depuis `estimation-ventes` : l'adresse est la première carte de l'onglet tant qu'elle manque ; le repère de l'adresse s'applique sans clic (« Annuler » ; un repère saisi à la main est conservé, « Remplacer par le repère de l'adresse ») ; le tableau par groupe montre minimum et maximum ; les ventes comparables (300 au plus) se trient, se filtrent (même immeuble, 2 dernières années, même nombre de pièces, DPE F ou G), se paginent par 20 et se déplient (prix de l'acte → actualisé → ramené au bien, Carrez, dépendances, terrain, lots, DPE probable rapproché par l'adresse, la surface à ±10 % et les 18 mois avant la vente, parcelle) ; aucune information sur les acheteurs ni les vendeurs (réidentification interdite par l'article R. 112 A-3 du LPF). Depuis `carte-interactive` : la carte des ventes se manipule sans piéger la page (Ctrl + molette, deux doigts au téléphone, un doigt fait défiler la page, plein écran fermé par Échap, « Recentrer sur le bien ») ; un clic sur une vente ouvre sa fiche (prix, surface et Carrez, prix au m² à la signature et d'aujourd'hui, distance, place, DPE probable, dépendances) avec « Voir dans le tableau », chaque ligne du tableau propose « Sur la carte », la vente choisie est mise en avant des deux côtés et les filtres du tableau s'appliquent aux pastilles ; un cercle de 100, 200 ou 300 m filtre le tableau ; les ventes du même immeuble forment une pastille chiffrée qui s'ouvre en éventail ; les couleurs disent le prix au m², l'ancienneté de la vente ou le DPE ; fonds Plan IGN ou photo aérienne, avec ou sans parcelles cadastrales (tuiles IGN gratuites, chargées seulement si on change de fond).
...
Feature `hypotheses-commandes` (fiche 24). Règle : **une hypothèse se change avec la même commande que dans le formulaire Vérifier.** Montants mis en forme (« 155 000 »), compteurs − / + (étage « RDC »), tuiles Oui / Non effaçables, tuiles de choix (état, tranche, régime), échelle DPE, année par période ou exacte, durée du prêt en tuiles 15 · 20 · 25 ans · Autre, apport en parts 0 · 10 · 20 % du coût total ou montant, nuits louées au curseur avec le taux d'occupation ; les taux restent une saisie texte (négatifs et centièmes). Même enveloppe (libellé, ⓘ, badge, aide ou erreur). Aucun calcul ni donnée changé.

## Site vitrine et guides (deklic.pro)

Feature `site-vitrine` (ADR-010). `deklic.pro` dit ce que fait Deklic, pour qui et ce que contient le rapport, puis mène à l'application sans compte : champ « Collez le lien de l'annonce » (ouvre Nouveau projet avec le lien), « Se connecter », « Gérer mes locations ». Le rapport d'exemple de l'accueil est calculé par le moteur. Les guides répondent aux recherches de calcul et de décision plutôt qu'au mot-clé « investissement locatif », tenu par les banques et les grands portails ; un mot-clé par guide, placé dans le title, le titre, l'adresse et les 100 premiers mots. Vague 1 (15/09/2026) : premier investissement locatif, calcul de la rentabilité locative, comment trouver un bien rentable, LMNP ou location nue, locataire qui ne paie pas son loyer, quittance de loyer. Chaque guide est signé Pierre Georgel, daté, sourcé par des pages officielles ; ses chiffres fiscaux viennent des règles du moteur. Vague 2 envisagée : cash-flow immobilier, révision du loyer (IRL), loi Jeanbrun (sans simulateur tant que le moteur ne la calcule pas). Pages « Où investir » par ville : plus tard. Jamais d'avis ni de note inventés.
...
- **v1 (8–10 sem.)** : extension + bookmarklet (LBC, SeLoger, Bien'ici, PAP, Logic-Immo) ; pipeline complet ; saisie manuelle ; 5 volets, verdict, scénarios ; sauvegarde locale, PDF, partage (lien court `/p/…` : copie sans la visite, 90 jours après la dernière ouverture, arrêt possible ; repli compressé sans serveur) ; test réel Marseille/Lyon/Aix.
...
- **Gérer (gestion locative après l'achat, épic G1 à G5, `specs/gestion-locative-specs.md`)** : commencé le 14/09/2026 avec le socle G1a (menu Analyser et Gérer, « Mon menu », biens, locataires, locations, loyers du mois, « Ajouter à la main » et « J'ai acheté ce bien » en deux clics, compte requis) ; puis G1b `quittances-fiches` (paiement en partie, quittance et reçu figés et imprimables selon l'art. 21 de la loi du 6 juillet 1989, identité du bailleur demandée une fois, page Loyers mois par mois, fiche d'un bien et fin de location, louer un bien vacant ou une autre chambre, plusieurs locataires par bien, export JSON ; paiements confirmés à la main en attendant la banque) ; puis G1c `gerer-biens` (pages Mes biens et Mes locataires, loyer modifié à partir d'un mois non payé, suppression d'un bien confirmée par son nom, APL versée au bailleur sur la ligne du loyer et la quittance, nom et e-mail du locataire corrigés) ; puis G1d `gerer-parcours` (pages reliées : fiche d'un locataire, chaque nom de bien ou de locataire mène à sa fiche, formulaire unique « Nouveau locataire » prérempli qui ramène à la page d'origine, quittance qui ramène à sa page d'origine, mois de la frise vers les loyers de ce mois, montant d'un loyer vers « Modifier », fil d'Ariane, bloc « À faire » en haut des loyers du mois) ; puis G2 `quittances-auto` (accord du locataire demandé par e-mail sans clic du bailleur et confirmé sur une page publique, ou déclaré par le bailleur ; quittance envoyée par e-mail avec son PDF quand un loyer est entièrement reçu, trace et « Renvoyer » ; téléphone du locataire ; bailleur propre à un bien, par exemple une SCI) ; puis, la nuit du 15 au 16/09/2026, `gerer-depenses-argent` (dépenses en deux clics avec récurrence, prêt du bien repris de l'analyse, page Argent : loyers encaissés, dépenses par catégorie, mensualités et cash-flow réel par mois ou par année, courbe des 12 mois, filtre par bien) ; puis la vie du bail B1 `gerer-bail-revision` (DPE et zone tendue du bien, alertes de conformité datées selon la loi Climat et résilience, révision annuelle du loyer par l'IRL avec gel des logements F et G, lettre de révision imprimable, jamais rétroactive) ; puis A2 `gerer-reel-declaration` (réel contre prévu sur la fiche d'un bien acheté après une analyse, « Analyser ce bien » pour les autres ; page Déclaration : montants à reporter par régime avec leurs lignes et cases, régime de l'analyse en premier, « à confirmer » tant que la notice de l'année n'est pas relue, rien de télédéclaré ; export CSV de l'année et récapitulatif imprimable pour le comptable ; export des données proposé avant la suppression du compte) ; ensuite G3 banque (détection des virements de loyer par les API bancaires, comme Rentila), G2 quittances automatiques, G4 vie du bail, G5 bilan et déclaration.
- **Acquisition** : site vitrine `deklic.pro` et six guides (livré le 15/09/2026) ; ensuite Search Console, guides de la vague 2, pages « Où investir » par ville.
