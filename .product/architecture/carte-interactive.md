...
Adresse.tsx
  └─ useLiaisonVentes(analyse) = useReducer(liaisonVentes) : { selection, afficher, demande, filtres, rayon }
       ...
       │    ├─ gestes : onWheelCapture (stopPropagation) / onTouchMoveCapture → messageGeste → message
       │    └─ CarteVentes (Leaflet, lazy) + calques-carte.ts : fond, parcelles, cercles cliquables, pastilles, éventail, bulle DOM
       └─ TableauVentes(analyse, liaison) : filtres et rayon contrôlés, ligne aria-current, « Sur la carte »
```
...
| Fichier                                           | Rôle                                                                                                                                                                                                                         |
| ------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `enrichissement/carte.ts`                         | + `FONDS_CARTE`, `COUCHES_FOND` (plan, photo : URL, attribution), `COUCHE_PARCELLES`, `RayonCarte`, `urlTuilesIgn(couche, style, format)`, `cleVente`, points avec `cle` et `vente: VenteProcheAdresse \| null`              |
...
| `ecrans/adresse/TableauVentes.tsx`                | reçoit `liaison` ; filtres et rayon contrôlés (`FiltresTableauVentes.tsx`), boutons de rayon, ligne `aria-current` et bouton « Sur la carte » (`LigneVente.tsx`), effet page + défilement + focus sur `afficher = 'tableau'` |
...
- **Gestes sans greffon** : React `onWheelCapture` sur le cadre arrête la propagation vers Leaflet sans Ctrl/⌘ hors plein écran (la page défile, défaut non empêché) ; `scrollWheelZoom` reste actif. Au doigt (`pointer: coarse`), `dragging` coupé hors plein écran : Leaflet pose alors `touch-action: pan-x pan-y` (classe `leaflet-touch-zoom` sans `leaflet-touch-drag`), un doigt fait défiler la page, deux doigts zooment et déplacent (TouchZoom). `onTouchMoveCapture` à un doigt → message (au glisser, pas au simple toucher : toucher une pastille ouvre sa fiche sans message) ; `glisserActif(auDoigt, pleinEcran)`.
- **Plein écran** : surcouche `fixed inset-0 z-[60]` sur le cadre, au-dessus de la barre latérale (`z-40`), pas d'API Fullscreen. Un seul bouton qui change de libellé (« Plein écran » / « Fermer la carte ») : le focus reste dessus. Échap : ferme d'abord la bulle, puis le plein écran.
