...
| `packages/capture` | Contrat de capture (schéma, encodage pour fragment d'URL, résolution d'URL) et moteur de règles (JSON-LD, état applicatif, meta, CSS)                     | **livré** (PR `feat/extension`, 68 tests, 100 %)                                    |
| `apps/extension`   | WebExtension MV3 : règles versionnées LBC, SeLoger, Bien'ici, PAP, Logic-Immo, popup « Analyser dans Deklic », script de contenu ; bouton-favori côté web | **livré** (PR `feat/extension`, 33 tests, 100 %) ; stores et règles sur R2 à venir  |
