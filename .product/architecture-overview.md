...
| `apps/web`         | Coque SaaS, Mes projets, Nouveau projet (capture, lecture IA, marché de la commune), Rapport, Hypothèses, Fiscalité, Revente, Visite                      | **livré** (PR #3 à #6, #16, enrichissement-marche) ; Comparer, Méthode, partage à venir |
| `apps/worker`      | `/proxy/*` (géocodage), `/extract` (lecture LLM des annonces), `/marche` (DVF, ANIL, zone ABC lus sur R2)                                                 | **livré et déployé** (PR #9, #10, #15, enrichissement-marche)                           |
