...
- **Fichiers lus** : `https://files.data.gouv.fr/geo-dvf/latest/csv/<annee>/departements/<dep>.csv.gz` (redirection vers un stockage S3 à suivre). Cinq dossiers annuels sont lus par département, du plus récent (détecté par une requête HEAD, ou imposé par `--millesime-dvf`) vers le passé : les CSV publiés couvrent 24 mois, la tendance des prix les cinq années. Le dossier de l'année en cours n'apparaît qu'en octobre, avec le premier semestre.
