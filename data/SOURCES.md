...
| --------------- | ----------------------------------------- | -------------------------------------------------------- | --------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `dvf`           | Ventes de logements géolocalisées         | DGFiP, retraitement Etalab                               | semestriel (avril, octobre) | `dvf/<millesime>/<codeInsee>.csv`, `dvf/<millesime>/index/<dep>.json`, `dvf/<millesime>/tendance/<dep>.json`, `dvf/<millesime>/index.json`, `dvf/courant.json` |
...
- **Fichiers lus** : `https://files.data.gouv.fr/geo-dvf/latest/csv/<annee>/departements/<dep>.csv.gz` (redirection vers un stockage S3 à suivre). Cinq dossiers annuels sont lus par département, du plus récent (détecté par une requête HEAD, ou imposé par `--millesime-dvf`) vers le passé : les CSV publiés par commune et la tendance des prix couvrent les cinq années (là où les ventes sont rares, les plus anciennes comptent ; le Worker les ramène au dernier semestre), l'index des prix les 24 derniers mois. Le dossier de l'année en cours n'apparaît qu'en octobre, avec le premier semestre.
...
- **Transformations** :
  ...
  3. Fenêtre de 24 mois se terminant à la dernière vente connue du département, bornes incluses : elle ne sert qu'à l'index (étape 5).
  4. `dvf/<millesime>/<codeInsee>.csv` : toutes les ventes retenues des cinq dossiers annuels, `date,prix,surface,type,pieces,lat,lon` (type `appartement` ou `maison`, coordonnées WGS-84 du centre de la parcelle, vides quand la parcelle n'est pas géocodée : la vente compte alors pour la commune, pas pour un rayon), ventes triées par date.
  5. `dvf/<millesime>/index/<dep>.json` : ventes de la fenêtre de 24 mois, par commune et par type, `ventes`, `medianeM2`, `q1M2`, `q3M2` (prix au m² arrondis à l'euro, quartiles par interpolation linéaire, méthode 7 de Hyndman et Fan) et la `fenetre` du département.
...
`aws s3 sync data/dist/<prefixe>/ s3://deklic-data/<prefixe>/ --endpoint-url https://<compte>.eu.r2.cloudflarestorage.com`, préfixe par préfixe, avec `--delete` seulement sur une passe France entière (une passe limitée à un département n'efface rien). Secrets GitHub attendus : `R2_ACCESS_KEY_ID`, `R2_SECRET_ACCESS_KEY` (jeton R2 « Object Read & Write » sur le bucket) et `CLOUDFLARE_ACCOUNT_ID`. Sans ces secrets, l'Action génère les fichiers et s'arrête avec un avertissement. Le choix de l'API S3 plutôt que de `wrangler r2 object put` tient au volume : environ 34 000 fichiers de commune par millésime DVF, qu'une commande par objet mettrait plus de dix heures à publier. Le bucket est dans la juridiction UE : les fichiers sont stockés dans l'Union européenne et ne sont accessibles que par l'adresse S3 européenne.
