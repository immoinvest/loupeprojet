...
- **Livré** : feature `gerer-socle` (épic « Gérer », G1a ; `.product/architecture/gerer-socle.md`, specs de l'épic `.product/specs/gestion-locative-specs.md`) : gestion locative après l'achat, pour un compte connecté. Menu à deux sections « Analyser » et « Gérer » (`coque/SectionAnalyser.tsx`, `SectionGerer.tsx`, styles `coque/liens.ts`, dans la zone qui défile de la barre latérale), carte « Mon menu » dans Mon compte pour masquer l'une ou l'autre (au moins une reste). Paquet pur `packages/gestion` (`@loupe/gestion` : montants en **centimes entiers**, dates civiles `AAAA-MM-JJ`, loyer dû du mois calculé et **jamais stocké**, prorata au jour, retard 5 jours après l'échéance, `resumeDuMois`). API `/api/gestion/*` dans `apps/comptes/src/gestion/` (même origine, session Better Auth, `Origin` exigé pour écrire, 64 Ko, 200 biens par compte, loyer reçu seulement pendant la location) sur la D1 `deklic-comptes`, migration `0002_gestion.sql` — **à appliquer en production par Pierre** (`npx wrangler d1 migrations apply deklic-comptes --remote` depuis `apps/comptes`) ; sans elle : 503 `GESTION_INDISPONIBLE`. Web : `src/gestion/` (`GestionProvider` / `useGestion`, clients réseau et mémoire, `AppEnMemoire({ gestion })`), écrans `ecrans/gerer/` : `/gerer` (sans compte, trois portes, loyers du mois, « Reçu » puis « Annuler »), `/gerer/ajouter`, `/gerer/pret/:id` (« J'ai acheté ce bien » depuis un projet, statut `achete`). **Règle des deux clics** prouvée par des tests qui comptent les clics. Ordre des features suivantes : G3 `banque` (API bancaires DSP2 pour détecter les virements, fournisseur et coût à décider avec Pierre) → G1b → G2 `quittances-auto` → G4 → G5.
...
| Données       | **KV** (cache API 24 h / extraction 30 j), **R2** (référentiels pré-agrégés, CSV DVF), **D1** (comptes : utilisateurs, sessions ; gestion locative : biens, locataires, locations, loyers reçus ; projets synchronisés ensuite) | Quotas gratuits largement suffisants                                                         |
