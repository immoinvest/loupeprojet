/** Styles partagés des liens de la barre latérale (sections Analyser et Gérer, aide). */
...
export const classeLienCreation = ({ isActive }: { isActive: boolean }): string =>
  `flex min-h-[44px] items-center gap-3 rounded-encart px-3.5 py-2.5 text-[15px] font-bold ${
    isActive ? 'bg-accent-doux text-accent' : 'text-accent hover:bg-accent-fond'
  }`;

export const CLASSE_ETIQUETTE =
  'px-3.5 pb-1.5 text-xs font-bold tracking-wider text-encre-4 uppercase';
