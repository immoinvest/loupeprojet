...
import { CLASSE_ETIQUETTE, CLASSE_SECTION, classeLien as lien } from './liens';
 ...
 * Chaque titre de section est collé à ce qu'il annonce et loin de ce qui le précède.
 */
export function Sidebar({
  ...
  return (
    <aside
      id={ID_NAVIGATION}
      data-ouvert={ouvert}
      className={`fixed inset-y-0 left-0 z-40 flex w-[min(20rem,85vw)] flex-col gap-3 overflow-hidden border-r border-bordure bg-surface pt-[max(1.25rem,env(safe-area-inset-top))] pr-3 pb-[max(1rem,env(safe-area-inset-bottom))] pl-[max(0.75rem,env(safe-area-inset-left))] transition-[translate,visibility] duration-200 motion-reduce:transition-none ${
    ...
    >
      <div className="flex shrink-0 items-center justify-between gap-2">
        {/* Le logo garde la main mais n'a pas de survol (décision de Pierre du 15/09/2026). */}
        <NavLink
          to="/"
          aria-label={TEXTES_LOGO}
          data-logo
          className="flex items-center rounded-encart px-2 py-1 pointer-coarse:min-h-11"
        >
      ...
      <nav aria-label={TEXTES_MENU.accueil} className="flex shrink-0 flex-col">
      ...
      <div
        data-zone="defilante"
        className="-mx-1 flex min-h-0 flex-1 flex-col gap-5 overflow-y-auto overscroll-contain px-1 pt-2"
      >
      ...
      <div className="flex shrink-0 flex-col gap-3 pt-2">
        <nav aria-label="Outils" className={CLASSE_SECTION}>
