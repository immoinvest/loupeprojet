...
export function sectionRevente(regles: Regles, defauts: Defauts): SectionMethode {
  ...
  return {
    ...
    etapes: [
      `Valeur ajoutée par les travaux = écart entre l'estimation du bien rénové et son estimation à l'état actuel, au prorata des travaux estimés pour cet état et jamais au-dessus de leur montant ; sans état du bien ni ventes comparables, ${pct(regles.revente.valorisationTravauxRepli)} des travaux (à confirmer). Valeur de revente = (prix retenu + valeur ajoutée par les travaux) × (1 + évolution annuelle)^années, ou le prix de vente que vous saisissez, quel que soit l'horizon ; par défaut ${pct(defauts.evolutionAnnuelle)} par an sur ${String(defauts.reventeAnnees)} ans. Frais de vente = agence ${pct(defauts.fraisAgenceTaux)} + diagnostics ${euros(defauts.diagnostics)}.`,
      'Cash net vendeur = valeur − frais de vente − capital restant dû − indemnité de remboursement anticipé − impôt sur la plus-value.',
      `Prix d'acquisition majoré = prix de l'acte + max(frais réels, ${pct(pv.forfaitFrais)}) + max(travaux réels, ${pct(pv.forfaitTravaux)} à partir de ${String(pv.forfaitTravauxDesAnnee)} ans de détention) − amortissements du bâti déduits en LMNP réel. Le prix de l'acte est le prix retenu sans les honoraires dus par l'acquéreur ; ces honoraires comptent dans les frais réels, et les deux forfaits se calculent sur le prix de l'acte (BOFiP, BOI-RFPI-PVI-20-10-20-20 § 40 et 70).`,
